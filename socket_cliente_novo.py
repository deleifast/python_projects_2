import socket, pickle, os, configparser, shutil, re, glob, time, wmi
...
info4 = {'name': ('lj') + loja + ('.') + ('cpu_') + caixa + ('.txt'), 'file': f_bin1} 

try:
	s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	s.connect(('172.16.252.88', 9005))
